	...
	main.addEventListener('focus', function(e){
    	Titanium.App.Analytics.trackPageview('/more/weather');
	});
function getGWeather(wx_location){
	...
	setTimeout(function()
	{
		...
		xhr.onload = function()
		{
			...
			data.push({title:'Location',color:'black',font:{fontSize:18,fontWeight:'bold',color:'white'},  backgroundColor:'#b3c5db',});
			...
			data.push({
				title:info.item(0).getElementsByTagName("city").item(0).getAttribute("data")+
					"\nData current as of "+wx_timestamp_array[0]+":"+wx_timestamp_array[1]+" on "+wx_time_array[0],color:'black',
			...
			data.push({title:'Current Conditions',color:'black',font:{fontSize:18,fontWeight:'bold'},backgroundColor:'#b3c5db'});			
			data.push({
				...
				leftImage:"http://www.google.com"+current.item(0).getElementsByTagName("icon").item(0).getAttribute("data"),color:'black',
				font:{fontSize:16,fontWeight:'normal'}
				});
			...
			data.push({title:'Forecast',color:'black',font:{fontSize:18,fontWeight:'bold'},backgroundColor:'#b3c5db'});
	        for (var i = 0; i < forecast.length; i++) {
	        	...
	        	data.push({
					...
					leftImage:"http://www.google.com"+forecast.item(i).getElementsByTagName("icon").item(0).getAttribute("data"),color:'black',
					font:{fontSize:16,fontWeight:'normal'}
					}); 

	        }
		actInd.hide();
		var table = Titanium.UI.createTableView({data:data,rowBackgroundColor:'white',backgroundColor:'white'});
